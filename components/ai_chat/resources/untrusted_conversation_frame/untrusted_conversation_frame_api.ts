    ...
    this.conversationObserver.onConversationHistoryUpdate.addListener(
      async (entry?: Mojom.ConversationTurn) => {
        if (entry) {
          ...
          const updatedHistory = updateConversationHistory(this.state.conversationHistory, entry)
          this.setPartialState({
            conversationHistory: updatedHistory
          })
        } else {
          // When no entry is provided, fetch the full history
          const { conversationHistory } = await this.conversationHandler.getConversationHistory()
