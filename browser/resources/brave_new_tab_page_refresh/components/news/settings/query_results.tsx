...
import { NewsPublisher, FindFeedResult } from '../../../models/news'
...
export function QueryResults(props: Props) {
  ...
  const [findFeedResults, setFindFeedResults] =
    React.useState<FindFeedResult[] | null>(null)
  ...
  const sourceMatches = React.useMemo(() => {
    const publisherMatches: NewsPublisher[] = []
    ...
    for (const result of findFeedResults || []) {
      ...
      const publisher = publisherFeedUrls.get(result.feedUrl)
      if (publisher) {
