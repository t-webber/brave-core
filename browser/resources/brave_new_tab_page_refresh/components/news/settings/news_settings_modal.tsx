...
export function NewsSettingsModal(props: Props) {
  ...
  const newsLocale = useAppState((s) => s.newsLocale)
  ...
  const popularPublishers = React.useMemo(() => {
    return Object.values(publishers)
      .map((publisher) => ({
        publisher,
        locale: publisher.locales.find((locale) => locale.name === newsLocale)
      }))
      .filter(({ locale }) => locale)
      .sort((a, b) => {
        const rankA = a.locale!.rank
        const rankB = b.locale!.rank
