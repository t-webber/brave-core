...
class PullToRefreshTabHelper: TabObserver, PreferencesObserver {
  ...
  private func updatePullToRefreshVisibility() {
    guard let url = tab?.visibleURL, let webView = tab?.webViewProxy else { return }
    webView.scrollView?.refreshControl =
      url.isLocalUtility || !Preferences.General.enablePullToRefresh.value ? nil : refreshControl
  }

  func tabDidUpdateURL(_ tab: any Tab) {
    updatePullToRefreshVisibility()
  }

  func tab(_ tab: any Tab, didCreateWebView webView: UIView) {
    updatePullToRefreshVisibility()
  }

  func tabDidStopLoading(_ tab: any Tab) {
    refreshControl.endRefreshing()
  }

  func tabWillBeDestroyed(_ tab: any Tab) {
