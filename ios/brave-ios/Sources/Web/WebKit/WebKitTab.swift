...
class WebKitTab: Tab, TabImpl {
  ...
  func userAgentForURL(_ url: URL) -> String? {
    let userAgentType = url.baseDomain.flatMap { userAgentOverrides[$0] } ?? .automatic
    return switch userAgentType {
    case .none, .automatic: UserAgent.userAgentForIdiom()
    case .mobile: UserAgent.mobile
    case .desktop: UserAgent.desktop
    }
  }
  ...
  private func webViewTitleDidChange() {
    ...
    {
      visibleURL = url  // We can update the URL whenever showing an interstitial warning
      observers.forEach {
        $0.tabDidUpdateURL(self)
      }
    }
  ...
  var currentUserAgentType: UserAgentType {
    ...
    return UserAgent.userAgentForIdiom() == UserAgent.mobile ? .mobile : .desktop
  }

  func loadRequest(_ request: URLRequest) {
    if let webView = webView {
      // FIXME: Have to find a way to donate somewhere else
