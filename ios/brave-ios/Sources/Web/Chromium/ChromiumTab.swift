...
class ChromiumTab: Tab, TabImpl {
  ...
  var visibleSecureContentState: SecureContentState {
    ...
    // FIXME: Likely need a check for brave:// and chrome:// urls here as well to return .localhost

    if let internalURL = InternalURL(lastCommittedURL) {
  ...
  var backForwardList: (any BackForwardListProxy)? {
    webView?.backForwardList.flatMap { ChromiumBackForwardList($0) }
  }
  ...
  var currentUserAgentType: UserAgentType {
    .init(webView?.currentItemUserAgentType() ?? .none)
  }
  ...
  func reload() {
    // FIXME: Test if calling standard `reload()` is valid if UA is altered in CWVWebView
    webView?.reload(withUserAgentType: .init(currentUserAgentType))
  }
  ...
  var observers: Set<AnyTabObserver> = []
  var policyDeciders: Set<AnyTabPolicyDecider> = []
}

extension CWVWebView: WebViewProxy {}

extension CWVUserAgentType {
  fileprivate init(_ userAgentType: UserAgentType) {
...
extension UserAgentType {
  fileprivate init(_ userAgentType: CWVUserAgentType) {
    switch userAgentType {
    case .none: self = .none
    case .automatic: self = .automatic
    case .desktop: self = .desktop
